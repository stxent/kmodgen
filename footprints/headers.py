...
class PinHeader(exporter.Footprint):
    def __init__(self, spec, descriptor):
        ...
        self.body_size = np.array([body_size_x, body_size_y])
        ...
        self.body_center = np.asarray(self.count - 1, dtype=np.float32)
    ...
    def generate_lines(self):
        ...
        outline_size = np.maximum(self.body_size, self.body_size + self.pad_size - outline_margin)
    ...
    def generate_pads(self):
        ...
        for x_offset in range(0, self.count[0]):
            for y_offset in range(0, self.count[1]):
                ...
                offset = np.array([float(x_offset), -float(y_offset)]) * self.pitch
                offset += np.array([0.0, self.pad_offset])
...
class AngularPinHeader(PinHeader):
    def __init__(self, spec, descriptor):
        ...
        self.body_size += np.array([0.0, projection_length])
        self.body_center += np.array([0.0, projection_length / 2.0])

    def generate(self):
        objects = self.generate_pads()
        objects.append(exporter.Label(self.name, self.body_center, self.thickness, self.font))

        edge_margin = np.array([0, self.thickness / 2.0])
        outline_margin = self.pitch - 2.0 * self.gap - self.thickness
        outline_size = np.maximum(self.body_size, self.body_size + self.pad_size - outline_margin)
        line_offset_x = outline_size[0] / 2.0
        line_offset_y = np.maximum(self.pitch / 2.0,
        ...
        objects.append(exporter.Line(
            np.array([line_offset_x + self.body_center[0], line_offset_y]),
            np.array([-line_offset_x + self.body_center[0], line_offset_y]),
            self.thickness
        ))
