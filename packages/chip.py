...
class ChipBase:
    ...
    def generate(self, materials, _, descriptor):
        ...
        meshes = ChipBase.make_chip(
            ...
            edge_resolution=ChipBase.EDGE_RESOLUTION,
            line_resolution=ChipBase.LINE_RESOLUTION
        )

        if self.material in materials:
            meshes[0].appearance().material = materials[self.material]
        if 'Pin' in materials:
            meshes[1].appearance().material = materials['Pin']
